...
public static class CardCmd
{
	public static async Task AutoPlay(PlayerChoiceContext choiceContext, CardModel card, Creature? target, AutoPlayType type = AutoPlayType.Default, bool skipXCapture = false, bool skipCardPileVisuals = false)
	{
		...
		if (!Hook.ShouldPlay(combatState, card, out AbstractModel preventer, type))
		{
			...
			{
				((Node)(object)card.Owner.Creature.GetVfxContainer())?.AddChildSafely((Node?)(object)NThoughtBubbleVfx.Create(playerDialogueLine.GetFormattedText(), card.Owner.Creature, 1.0));
			}
			return;
		}
	...
	{
		await Discard(choiceContext, new global::<>z__ReadOnlySingleElementList<CardModel>(card));
	}
	...
	{
		Upgrade(new global::<>z__ReadOnlySingleElementList<CardModel>(card), style);
	}
	...
	private static TaskCompletionSource? PreviewInternal(CardModel card, bool isAddingCardsToPile, IEnumerable<RelicModel>? relicsToFlash = null, float time = 1.2f, CardPreviewStyle style = CardPreviewStyle.HorizontalLayout)
	{
		...
		if (style == CardPreviewStyle.HorizontalLayout && ((Node)val2).GetChildCount(false) > 5)
		{
			val2 = (Control)(object)(pileType.IsCombatPile() ? NCombatRoom.Instance.Ui.MessyCardPreviewContainer : NRun.Instance?.GlobalUi.MessyCardPreviewContainer);
		}
		NCard node = NCard.Create(card);
		((Node)(object)val2)?.AddChildSafely((Node?)(object)node);
		node.UpdateVisuals(pileType, CardPreviewMode.Normal);
		TaskCompletionSource source = new TaskCompletionSource();
		Tween val3 = ((Node)node).CreateTween();
		val3.TweenProperty((GodotObject)(object)node, NodePath.op_Implicit("scale"), Variant.op_Implicit(Vector2.One), 0.25).From(Variant.op_Implicit(Vector2.Zero)).SetEase((EaseType)1)
		...
		val3.TweenCallback(Callable.From((Action)delegate
		{
			...
			Node val4 = (Node)((pileType != PileType.Deck) ? ((object)card.Owner.Creature.GetVfxContainer()) : ((object)NRun.Instance?.GlobalUi.TopBar.TrailContainer));
			if (val4 != null)
			{
			...
			if (nCardFlyVfx != null && val4 != null)
			{
				val4.AddChildSafely((Node?)(object)nCardFlyVfx);
	...
	private static int GetTotalCardsBeingPreviewed()
	{
		...
		{
			num = ((Node)NCombatRoom.Instance.Ui.CardPreviewContainer).GetChildCount(false);
			num += ((Node)NCombatRoom.Instance.Ui.MessyCardPreviewContainer).GetChildCount(false);
		}
		if (NRun.Instance != null)
		{
			num += ((Node)NRun.Instance.GlobalUi.CardPreviewContainer).GetChildCount(false);
			num += ((Node)NRun.Instance.GlobalUi.MessyCardPreviewContainer).GetChildCount(false);
			num += ((Node)NRun.Instance.GlobalUi.EventCardPreviewContainer).GetChildCount(false);
			num += ((Node)NRun.Instance.GlobalUi.GridCardPreviewContainer).GetChildCount(false);
		}
		return num;
	}
