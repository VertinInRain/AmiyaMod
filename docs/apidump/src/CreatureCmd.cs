...
public static class CreatureCmd
{
	...
	public static async Task<IEnumerable<DamageResult>> Damage(PlayerChoiceContext choiceContext, IEnumerable<Creature>? targets, decimal amount, ValueProp props, Creature? dealer, CardModel? cardSource, CardPlay? cardPlay)
	{
		...
		foreach (Creature originalTarget in targetList)
		{
			...
			foreach (DamageResult item in damageResults)
			{
				...
				Node vfxContainer = (Node)(object)receiver.GetVfxContainer();
		...
		foreach (DamageResult unblockedDamageResult in results)
		{
			...
			if (unblockedDamageResult.WasFullyBlocked && CombatManager.Instance.IsInProgress)
			{
				...
				Node vfxContainer2 = (Node)(object)unblockedDamageResult.Receiver.GetVfxContainer();
				vfxContainer2?.AddChildSafely((Node?)(object)NBlockSparkVfx.Create(unblockedDamageResult.Receiver));
				vfxContainer2?.AddChildSafely((Node?)(object)NDamageBlockedVfx.Create(unblockedDamageResult.Receiver));
				NGame.Instance?.ScreenShake(ShakeStrength.Weak, ShakeDuration.Short);
			}
		}
	...
	public static async Task Heal(Creature creature, decimal amount, bool playAnim = true)
	{
		...
		if (playAnim)
		{
			if (CombatManager.Instance.IsInProgress || creature.Player?.RunState.CurrentRoom is CombatRoom)
			{
				...
				((Node)(object)creature.GetVfxContainer())?.AddChildSafely((Node?)(object)NHealNumVfx.Create(creature, amount));
				if (wasDead)
				{
	...
	public static Task Stun(Creature creature, Func<IReadOnlyList<Creature>, Task> stunMove, string? nextMoveId = null)
	{
		...
		async Task Wrapper(IReadOnlyList<Creature> c)
		{
			...
			if (vfx != null)
			{
				Node vfxContainer = (Node)(object)creature.GetVfxContainer();
				if (vfxContainer != null)
				{
					Callable val = Callable.From((Action)delegate
					{
						vfxContainer.AddChildSafely((Node?)(object)vfx);
					});
